...
void feed_forward(MTX &x, MTX &w, MTX &sy, MTX &y){
   int m = w.size();
   int n = w.front().size();
...
void a_sample(MTX &ash, MTX &sh, MTX &ah, MTX &h, MTX &asy, MTX &sy){
   switch(m_s){
      case DET:
         for (int k=0; k<batch; k++)
            for (int j=0; j<n_s; j++)
      ...
      case GUMBEL:
         ...
         break;
      case LR:
         int m = sy.front().size()/n_s;
         for (int k=0; k<batch; k++)
            for (int j=0; j<n_s; j++)
               for (int i=0; i<h_d; i++)
                  for (int l=0; l<m; l++)
                     ah[k][i] += sy[k][i+j*m] / h[k][i];
         break;
   }
}
