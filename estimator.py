...
def mse1(p, x, t=0.1, c=0., d=1, kk=0):
    k = kk*p+ .3
    u0a, u1a = 1., p-k
    ...
    return tf.where(x<w1, -1./d*tf.log(x1/w1*(tf.exp(d*u1a)-tf.exp(d*u0a))+tf.exp(d*u0a)) + u0a + u1a, u0a*tf.ones(tf.shape(x)))

def mse0(p, x, t=0.1, c=0., d=1, kk=0):
    k = kk*p+ .3
    u0b, u1b = 1-p+k, 0.
    ...
    return tf.where(x>w0, 1./d*tf.log((1-x0)/(1-w0)*(tf.exp(d*u1b)-tf.exp(d*u0b))+tf.exp(d*u0b)), u1b*tf.ones(tf.shape(x)))

def mse(p, x, t=0.1, c=0., d=1, kk=0):
    s = tf.where(x<p+c, mse1(p,x,t,c,d,kk), mse0(p,x,t,c,d,kk))
